...
Source `acceptance/private/7.28引江济淮郸城配套项目一体化泵站询比文件.pdf` (`8e2bfb00e1a0…`) → build `acceptance/workspace/case_001/v1_manual_fidelity_round3_word_review_final`.

**Totals**

- source date rows **12** (build scope 40–61: **10**); generated date paragraphs **10**
- alignment matched **9**, reviewed exceptions **2**, alignment mismatch **1**; collapsed rows **7**; lost source gaps **21**; invented gaps **0**; unexpected date indents **2**; ambiguous **0**
...
- SYNTHETIC_CENTER_INDENT — src p40 is centred (offset +11.46 pt vs tol 11.91 pt) but docx ¶6 is w:jc=left + w:left=212.40 pt (w:tabs [212.4, 264.2, 321.7] pt).
- NATIVE_CENTER_BUT_NO_SPACING — src p47/53/60 are centred natively (docx ¶86,114,136 w:jc=center, w:left=0) yet still drop the 21.96 pt intra-row gaps.
- POSITION_BY_INDENT_ONLY — 4 anchored rows (docx ¶45,50,76,91) reproduce source x with w:left only and lose every internal gap.
- TAB_STOPS_EXIST_BUT_MISMATCH — docx ¶6,57,64 carry w:tabs/underlined leader tabs but 7 of 9 material gap positions miss the 2.0 pt tolerance.
- GENERATION_REPORT_NOT_AUTHORITATIVE — generation_report.json records tab_stops=[] for docx ¶6,57,64 although the emitted docx contains 3/4/4 w:tabs.
...
| 40 | 635.41 | ALIGNED_CENTER | 309.11 / +11.46 | 6 | left | 4248 tw | 46.00 / 26.80 | 40.34 / 45.98 | no | no | ALIGNMENT MISMATCH; synthetic w:left for centring; gaps lost: before_year,year_month,month_day |
| 42 | 586.14 | ANCHORED_FORM_ROW | 247.80 / -49.85 | 45 | left | 2940 tw | 0.00 / 0.00 | 12.00 / 12.00 | no | yes | collapsed: no gap mechanism |
| 43 | 659.34 | ANCHORED_FORM_ROW | 462.60 / +164.95 | 50 | left | 7166 tw | 0.00 / 0.00 | 15.60 / 15.36 | no | yes | collapsed: no gap mechanism |
...
| 45 | 466.14 | ANCHORED_FORM_ROW | 387.72 / +90.07 | 76 | left | 5738 tw | 0.00 / 0.00 | 12.00 / 12.00 | no | yes | collapsed: no gap mechanism |
| 47 | 519.66 | ALIGNED_CENTER | 309.36 / +11.71 | 86 | center | 0 tw | 0.00 / 0.00 | 21.96 / 21.96 | no | yes | collapsed: no gap mechanism |
| 48 | 688.98 | ANCHORED_FORM_ROW | 393.84 / +96.19 | 91 | left | 5662 tw | 0.00 / 0.00 | 21.96 / 21.96 | no | yes | collapsed: no gap mechanism |
| 53 | 576.18 | ALIGNED_CENTER | 309.36 / +11.71 | 114 | center | 0 tw | 0.00 / 0.00 | 21.96 / 21.96 | no | yes | collapsed: no gap mechanism |
| 60 | 428.10 | ALIGNED_CENTER | 309.36 / +11.71 | 136 | center | 0 tw | 0.00 / 0.00 | 21.96 / 21.96 | no | yes | collapsed: no gap mechanism |
...
| docx | ¶15 | — | `七、近年（2023 年 1 月 1 日以来）类似项目情况表` | prose row: 26 glyphs besides 年/月/日 |
| docx | ¶108 | — | `供应商须自行承诺，近三年（2023 年 1 月 1 日）以来企业、法定代` | prose row: 90 glyphs besides 年/月/日 |
| docx | ¶110 | — | `七、近年（2023 年 1 月 1 日以来）类似项目情况表` | prose row: 26 glyphs besides 年/月/日 |
...
- PAIRING: source row <-> generated paragraph pairing uses generation_report.json paragraph_layout_records[paragraph_index].source_page + source_baselines[0] (role == DATE_LINE) for all 10 pairs; the generated PDF row is attached by document order, which is safe because both sequences are monotone over generated pages 1,3,4,5,5,6,8,9,14,21. pairing_token_x_delta_pt is recorded per row as the residual check (max |delta| = 51.90 pt on source page 40, where the row is also shifted by a synthetic indent).
